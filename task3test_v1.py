...
class Net(nn.Module):
    def __init__(self):
        ...
        self.fc1 = nn.Linear(64*4*4, 1536)
    ...
    def test(self, net, testSample, network, testIdx):
        ...
        if testIdx == 0:
            f = open("testresults_task3_v1.txt", "a")
...
def main():
    open('testresults_task3_v1.txt', 'w').close()
    net = Net()
    network = torch.load('./Saved_Networks/bestNetwork_task3_v1.pth')
